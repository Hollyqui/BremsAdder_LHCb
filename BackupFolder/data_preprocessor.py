...
def compose_df(df, filtered_x, filtered_y, scaling_arr, weighting, p_clusters_x, energy, daughter_clusters_x,
                           rot_cen_velo_state_x, rot_cen_velo_state_y, velo_state_z, rot_cen_tt_state_x, rot_cen_tt_state_y, ttrack_state_z):
    ...
    scaling = []
    frame = []
    velo_x, velo_y, velo_z, tt_x, tt_y, tt_z = [],[],[],[],[],[]
    for i in range(len(filtered_x)):
        for j in range(len(filtered_x[i])):
            scaling.append(scaling_arr[i])
            frame.append(weighting[i][j])
            velo_x.append(rot_cen_velo_state_x[i])
            velo_y.append(rot_cen_velo_state_y[i])
            velo_z.append(velo_state_z[i])
            tt_x.append(rot_cen_tt_state_x[i])
            tt_y.append(rot_cen_tt_state_y[i])
            tt_z.append(ttrack_state_z[i])
    X_train['weighting'] = frame
    X_train['scaling'] = scaling
    print(X_train.shape)
    print(rot_cen_velo_state_x.shape)
    X_train['velo_state_x'] = velo_x
    X_train['velo_state_y'] = velo_y
    X_train['velo_state_z'] = velo_z
    X_train['tt_state_x'] = tt_x
    X_train['tt_state_y'] = tt_y
    X_train['tt_state_z'] = tt_z
    is_phot = []
    for i in range(len(filtered_x)):
        for j in range(len(filtered_x[i])):
...
def return_candidate_frame(df, DAUGHTER_CLUSTER_MATCH_MAX=75, n_candidate = 10):
    ...
    centered_ttrack_state_x, centered_ttrack_state_y = center_values(center, [ttrack_state_x, ttrack_state_y])
    centered_velo_state_x, centered_velo_state_y = center_values(center,[velo_state_x, velo_state_y])
    ...
    rot_cen_tt_state_x, rot_cen_tt_state_y = rotate([centered_line_x, centered_line_y], [centered_ttrack_state_x, centered_ttrack_state_y])
    rot_cen_velo_state_x, rot_cen_velo_state_y = rotate([centered_line_x, centered_line_y], [centered_velo_state_x, centered_velo_state_y])
    ...
    for i in range(len(centered_clusters_x)):
        ...
        rot_cen_photon_clusters_x[i], rot_cen_photon_clusters_y[i] = norm([rot_cen_line_x[i],rot_cen_line_y[i]], [rot_cen_photon_clusters_x[i], rot_cen_photon_clusters_y[i]])
    rot_cen_line_x, rot_cen_line_y = norm([rot_cen_line_x,rot_cen_line_y],[rot_cen_line_x, rot_cen_line_y])
    rot_cen_tt_state_x, rot_cen_tt_state_y = norm([rot_cen_line_x,rot_cen_line_y], [centered_ttrack_state_x, centered_ttrack_state_y])
    rot_cen_velo_state_x, rot_cen_velo_state_y = norm([rot_cen_line_x,rot_cen_line_y], [centered_velo_state_x, centered_velo_state_y])
    # plot_graph(clusters_x=rot_cen_clusters_x, clusters_y=rot_cen_clusters_y, photon_clusters_x=rot_cen_photon_clusters_x, photon_clusters_y=rot_cen_photon_clusters_y)
