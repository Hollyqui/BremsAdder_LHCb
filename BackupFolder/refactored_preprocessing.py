...
def get_non_arrays(df):
    ...
    try:
        ...
        single_X_train['velo_ttrack_angle'] = get_angle(tt_dir, velo_dir)
    except:
...
def return_calculation_frame(model, cand_frame, orig_df, n_candidate=10, n_passed=3):
    ...
    for i in range(len(cand_frame)):
        ...
        # sort photons for that event
        if i%n_candidate == 0:
            x_pos = np.concatenate((x_pos,[x for _,x in sorted(zip(probs.T[0][i:i+10],cand_frame['x_pos'][i:i+10]))][:n_passed]))
            y_pos = np.concatenate((y_pos,[x for _,x in sorted(zip(probs.T[0][i:i+10],cand_frame['y_pos'][i:i+10]))][:n_passed]))
            weighting = np.concatenate((weighting,[x for _,x in sorted(zip(probs.T[0][i:i+10],cand_frame['weighting'][i:i+10]))][:n_passed]))
            cluster_sprx = np.concatenate((cluster_sprx,[x for _,x in sorted(zip(probs.T[0][i:i+10],cand_frame['cluster_sprx'][i:i+10]))][:n_passed]))
            cluster_spry = np.concatenate((cluster_spry,[x for _,x in sorted(zip(probs.T[0][i:i+10],cand_frame['cluster_spry'][i:i+10]))][:n_passed]))
            cluster_cl = np.concatenate((cluster_cl,[x for _,x in sorted(zip(probs.T[0][i:i+10],cand_frame['cluster_cl'][i:i+10]))][:n_passed]))
            cluster_pt = np.concatenate((cluster_pt,[x for _,x in sorted(zip(probs.T[0][i:i+10],cand_frame['cluster_pt'][i:i+10]))][:n_passed]))
            energy = np.concatenate((energy,[x for _,x in sorted(zip(probs.T[0][i:i+10],cand_frame['energy'][i:i+10]))][:n_passed]))
            probs_sort = np.concatenate((probs_sort,[x for _,x in sorted(zip(probs.T[0][i:i+10],probs.T[0][i:i+10]))][:n_passed]))

    origins = np.array(origins)
    ph_origin_x = origins[:,0][:,0]
    ph_origin_y = origins[:,1][:,0]
    ph_origin_z = np.clip(origins[:,2][:,0], 0, magnet_start)


    dz = showermax-ph_origin_z
    dx = x_pos-ph_origin_x
    dy = y_pos-ph_origin_y
    ...
    orig_gradient = np.sqrt(x_pos**2+y_pos**2)/showermax
    new_gradient = np.sqrt(dx**2+dy**2)/abs(dz)


    photon_data = pd.DataFrame({"xgb_prob_daughter": 1-probs_sort})
    photon_data['x_pos'] = x_pos
    photon_data['y_pos'] = y_pos
    # photon_data['weighting'] = weighting
    photon_data['cluster_sprx'] = cluster_sprx
    photon_data['cluster_spry'] = cluster_spry
    # photon_data['cluster_cl'] = cluster_cl
    photon_data['cluster_pt'] = cluster_pt
    photon_data['scaled_pt'] = (new_gradient/orig_gradient)*cluster_pt
    photon_data['energy'] = energy
    photon_data['scaled_pt_times_likelihood'] = photon_data['scaled_pt']*photon_data['xgb_prob_daughter']
    photon_data['gamma_sx'] = energy * gamma * sx
    photon_data['gamma_sy'] = energy * gamma * sy
    photon_data['gamma_sz'] = energy * gamma * sz
    # 'squish' all 10 candidate photons into a single row
    np_p_data = np.array(photon_data)
    np_p_data = np.reshape(np_p_data, (len(photon_data)//n_passed,len(np_p_data[0])*n_passed))
