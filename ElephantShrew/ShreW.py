...
xgb_shrew = xgb.XGBRegressor(objective='reg:squarederror', max_depth=8, learning_rate=0.3, batch_size=32, verbosity=2, n_estimators=5000, min_child_weight=100,
                         reg_alpha=0.2, reg_lambda=0.3, subsample=0.8, gamma=50000)
...
xgb_shrew.save_model("xgb2_Shrew.model")
...
kernel = 1**2*RBF(1e6, (1e4, 1e7))#ConstantKernel(1e7, (1e4, 1e7))
gpr = GaussianProcessRegressor(kernel=kernel, normalize_y=True, alpha=5000, optimizer='fmin_l_bfgs_b',n_restarts_optimizer=3)
...
svr_rbf.fit(np.array(x_train), y_train)
svm_shrew_train = svr_rbf.predict(x_train)


plt.hist(calc_frame['TRUE_P']-svm_shrew_train, range = (-30000,30000), bins=500, alpha=0.5, color='orange')
...
plt.hist(calc_frame_test['TRUE_P']-calc_frame_test['eminus_P'], range = (-range_value,range_value), bins=500, alpha=0.5, color='blue')
...
plt.hist(calc_frame_test['TRUE_P']-calc_frame_test['eminus_P'], range = (-30000,30000), bins=500, alpha=0.5, color='blue')
gpr_mse_test = np.average((calc_frame_test['TRUE_P']-gpr_preds_test)**2)
gpr_std_test = np.std(calc_frame_test['TRUE_P']-gpr_preds_test)
...
svr_preds_test = svr_rbf.predict(np.array(x_test))
plt.hist(calc_frame_test['TRUE_P']-svr_preds_test, range = (-30000,30000), bins=500, alpha=0.5, color='orange')
plt.hist(calc_frame_test['TRUE_P']-df_test['eminus_P'], range = (-30000,30000), bins=500, alpha=0.5, color='blue')
svr_mse_test = np.average((calc_frame_test['TRUE_P']-svr_preds_test)**2)
svr_std_test = np.std(calc_frame_test['TRUE_P']-svr_preds_test)
